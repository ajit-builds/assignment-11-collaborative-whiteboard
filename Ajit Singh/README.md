...
> **Repository:** [https://github.com/ajitsingh/assignment-11-collaborative-whiteboard](https://github.com/ajitsingh/assignment-11-collaborative-whiteboard)  
...
|---|:---:|---|---|
| `board:join` | Client -> Server | `{ "boardId": "DESIGN_101", "username": "Ajit", "userColor": "#ff5722" }` | Join a collaborative room |
| `board:init` | Server -> Client | `{ "strokes": [...], "activeUsers": [...] }` | Emits stroke history and active users to newly joined peer |
| `user:joined` | Server -> Room | `{ "userId": "socket_id", "username": "Ajit", "color": "#ff5722" }` | Notifies other room participants |
| `user:left` | Server -> Room | `{ "userId": "socket_id", "username": "Ajit" }` | Broadcasted when a peer disconnects |
...
| `cursor:update` | Server -> Room | `{ "userId": "socket_id", "username": "Ajit", "color": "#ff5722", "x": 140, "y": 95 }` | Relays peer cursor positions on screen |
| `board:clear` | Client -> Server | `{ "boardId": "DESIGN_101" }` | Clears all strokes for this room |
| `board:cleared` | Server -> Room | `{ "clearedBy": "Ajit" }` | Notifies room peers to clear canvas |
